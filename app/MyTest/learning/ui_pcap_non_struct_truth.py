...
def get_data(txt):
    ...
    for appdir in appdict.keys():
        if re.search('/0/', appdir):
            os.path.walk(appdir, visit, [titles, titles_label, '1', titles_pcap])
        elif re.search('/1/', appdir):
            os.path.walk(appdir, visit, [titles, titles_label, '2', titles_pcap])
...
train_file = open('train_ui_pcap_non_allFeature_truth_struct.arff', 'w')
