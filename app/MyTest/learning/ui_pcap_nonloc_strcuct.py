...
def get_data(txt):
    ...
    for appdir in appdict.keys():
        os.path.walk(appdir, visit, [titles, titles_label, appdict[appdir][1], titles_pcap])
    os.path.walk(non_loc_dir, visit, [titles, titles_label, 0, titles_pcap]) # for non-loc data
    ...
    if len(titles_label) != len(titles):
        ...
        exit(1)
    weka(vocab, dist, train_data_features, total_words, titles_label, titles_pcap)

def weka(vocab, dist, train_data_features, total_words, titles_label, titles_pcap):
    train_file.write('@RELATION ' + 'pcap' + '\n')
    train_file.write('@ATTRIBUTE pcapname STRING\n')
    ...
    for title_counts in train_data_features:
        ...
        # print title_counts
        #word_freq_list = []
            train_file.write(titles_pcap[counter] + ',')
            for word_count in title_counts:
                # calculate freq of words = percentage of words in front page that match WORD
                # i.e. 100 * (number of times the WORD appears in the front_page) /  total number of words in front page
...
def str2words(str, wordlist):
    ...
    for word in words:
        if len(word) < 13:# and word in url_words:
            wordlist.append(word)
    return ' '.join(words)

# read from txt and get url
def struct_fea_cal(dirname, filename, titles, titles_label, ui_label, titles_pcap):
    pcapreader = csv.reader(open(dirname + '/' + filename, 'rb'), delimiter='\t')
    result = list(pcapreader)
    ...
    titles.append(' '.join(wordlist))
    titles_label.append(ui_label)
    titles_pcap.append(dirname + '/' + filename)
    return True
...
def visit(arg, dirname, files):
    ...
    for filename in files:
        if re.search('\.csv', filename) or (re.search('\.c1sv', filename) and re.search('/0', dirname)):
            ...
            struct_fea_cal(dirname, filename, titles, titles_label, ui_label, titles_pcap)
...
train_file = open('train_ui_pcap_non_allFeature_struct.arff', 'w')
