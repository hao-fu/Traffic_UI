...
% 
train_csv =  csvread('train_ui_pcap_ocsvm_all_few.csv', 1);
...
% cut_point = ceil(size(trainData, 1) * 0.66);
% trainIndex = zeros(N, 1); trainIndex(1: cut_point) = 1; % first cut_point number as training samples
% crosstestIndex = zeros(N, 1); crosstestIndex(cut_point + 1: size(trainData, 1)) = 1;
% crosstestData = trainData(crosstestIndex==1,:);
% trainData = trainData(trainIndex==1,:);
% trainLabel = ones(size(trainData, 1), 1);
% crosstestLabel = ones(size(crosstestData, 1), 1);
...
%[predicted_label, accracy, a] = svmpredict(crosstestLabel, crosstestData, model);
